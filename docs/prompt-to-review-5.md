...
You don't write a single line of code until we align on a plan. And you don't call anything "done" until it meets rigorous quality criteria. You are committed to operate as a meticulous, transparent technical partner committed to exceptional thoroughness, systematic planning, and the delivery of optimal, maintainable solutions that reject generic aesthetics in favor of intentional, bespoke design. As a good practice, do a local git commit after a major milestone.

Now, please clone the GitHub repo using `git clone https://github.com/nordeim/stillwater.git`, then meticulously review the included `design.md` , `PAD.md` , `stillwater_SKILL.md` , `MASTER_EXECUTION_PLAN.md` , `Project_Brief.md` to have a deep understanding of the WHAT, WHY and HOW of the project and its codebase design and architecture. Next, please meticulously analyze these project design and specification documents carefully to identify potential conflicts and discrepancies. The project specific `stillwater_SKILL.md` was distilled and condensed from the source skills in the included `skills/` folder (see `skills/skills-catalog.md` for a list of included skills), with any conflicts to be resolved by extensive web searches to establish the ground truth. Next, scan the cloned project folder for completed codebase files to establish the current project status so that we know where we are and how to progress from here.

Next, meticulously review the included `pnpm_log.txt` to check for any server start issues.

Next, meticulously plan to use the relevant skills in the included `skills/` folder (refer to the included `skills/skills-catalog.md` for list of skills included in the cloned repo) to conduct a systematic and thorough code review and audit of the cloned codebase. For browser-based E2E tests, point the 'agent-browser' tool to URL `https://stillwater.jesspete.shop/` which is a live site running the current project codebase.
