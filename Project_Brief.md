...
> **Updated:** 2026-07-24 (post-v20 CSS cascade + env-validator remediation)
> Status: Phases 0–12 ✅ complete · v8–v12 ✅ · v13 ✅ (Six-Axis Audit) · v14 ✅ (Mockup fidelity) · v15 ✅ · **v16-1 to v16-3 ✅** (Loading… definitively fixed — live site fully operational) · **v17-1 to v17-10 ✅** (11 audit findings remediated via TDD — see AUDIT_REMEDIATION.md §v17) · **v19 ✅** (22 fixes: invisible tokens, slug-as-name, email hardcoded URLs, PaymentFailed payload — see AUDIT_REMEDIATION.md §V19) · **v20 ✅** (7 fixes: CSS cascade @layer base, /schedule nested eager-load, auth env fail-fast — see AUDIT_REMEDIATION.md §V20)
...
- `packages/db/drizzle/migrations/` — 6 migrations: `0000_dear_dagger.sql` (initial 18-table schema), `0001_equal_iron_lad.sql` (instructors.published column), `0002_lyrical_cargill.sql` (waitlist unique index `idx_waitlist_session_member`), `0003_audit_log_phase9.sql` (audit_log table), `0004_huge_hawkeye.sql` (enrollments `reminder_24h_sent_at` / `reminder_1h_sent_at` for cron-reminder dedup), `0005_add_price_cents.sql` (membership_plans.price_cents). All six applied successfully via `pnpm db:migrate`.
...
- `packages/db/src/index.test.ts` + `schema/*.test.ts` — 18 test files, 131 tests.

### Authentication & Authorization
- `packages/auth/src/config.ts` — Better Auth config (Google OAuth, Magic Link, Drizzle adapter, V20-4 env var fail-fast validator).
...
- `packages/auth/src/resend-client.ts` — Resend email client with V20-4 fail-fast for missing `RESEND_API_KEY`.
...
- `packages/auth/src/*.test.ts` — 5 test files, 107 tests (104 passing, 3 failing — V20-4 env-validator tests timeout due to drizzle adapter initialization during dynamic import).
...
- `packages/api/src/*.test.ts` — 14 test files, 150 tests (integration test requires Docker).
...
- `apps/web/src/**/*.test.ts` + `apps/web/src/**/*.test.tsx` — 38 test files, 271 tests.
...
- `services/workers/src/*.ts` — 12 task files + barrel `index.ts`: `attendance-summary`, `booking-confirmation`, `booking-cancellation`, `class-cancellation-notify`, `class-reminder-1h`, `class-reminder-24h`, `membership-credit-grant`, `membership-expiry-warn`, `payment-failed-notify`, `waitlist-expiry`, `waitlist-promotion`, `weekly-digest` (each with per-task `maxDuration` + retry).
- `services/workers/src/*.test.ts` — 12 test files, 45 tests.
...
## Live quality gates (2026-07-24, post-v20 remediation)
...
| `pnpm lint` | **0 errors** ✅ (10 intentional warnings — `no-console` in logger.ts) |
| `pnpm test` | **832 tests (829 passing, 3 failing)** ⚠️ — 131 db + 107 auth (3 V20-4 env-validator failures) + 150 api + 47 payments + 10 config + 271 web + 71 email + 45 workers |
| `pnpm build` | **✅ 9/9 packages, 20 static pages** (verified 2026-07-24) ✅ |

> ⚠️ **3 auth test failures (V20-4):** The env-validator tests for `BETTER_AUTH_URL`, `GOOGLE_CLIENT_ID`, and `GOOGLE_CLIENT_SECRET` fail because `vi.resetModules()` + `import('./config')` triggers the drizzle adapter DB connection before the env check runs, causing timeouts. The fail-fast logic itself is correct in `config.ts` — only the test harness needs fixing.
...
Test breakdown (run `pnpm test` for the authoritative count):
- `packages/db` — 18 test files / **131 tests**
- `packages/auth` — 5 test files / **107 tests** (3 V20-4 env-validator failures — see note above)
- `packages/api` — 14 test files / **150 tests** (V13-2 cancel + V13-4 RBAC + V13-5 credit + V17-4 cartesian-join fix + V17-6 ilike escape + V19-4/5/6 eager-load)
- `packages/payments` — 7 test files / **47 tests** (V13-6 checkout.session.completed + charge.refunded)
- `packages/config` — 1 test file / **10 tests** (V17-8 SITE constants + V19-14 SITE.url)
- `apps/web` — 38 test files / **271 tests** (V13-1 index-routes + V14 Footer + slug-404-verify + V17-2 CSP rewrite + V17-3 HeroNextClass CLS + V17-5 instructor title + V17-7 studio layout + V17-10 SSE rate limit + V19-1 sand tokens + V20-1 globals layer)
- `packages/email` — 17 test files / **71 tests**
- `services/workers` — 12 test files / **45 tests** (V13-3 claim URL domain fix + V19-17 PaymentFailed payload)

Build output: marketing routes (8: home, schedule, instructors×2, pricing, blog×2, about) + legal (3: privacy, terms, accessibility) + studio (5) + admin (11, RBAC-gated) + auth (4: sign-in, sign-out, callback, error) + API routes (trpc, auth catch-all, schedule/stream, sanity/webhook, `/api/webhooks/stripe`, admin/members/export). V16-1: `/`, `/schedule`, `/pricing` are `force-dynamic` (ƒ) to prevent prerender Suspense hang. `/instructors`, `/about`, `/blog`, `/privacy`, `/terms`, `/accessibility` are static (○). V16-2: React Compiler disabled. V16-3: CSP `strict-dynamic` removed. V17-1: leaked `env.local` files removed from git tracking. V17-2: CSP tests rewritten to verify behavior. V17-3: CLS fix (HeroNextClass skeleton + Cormorant font-display optional). V17-4: getRevenueDetails cartesian-join bug fixed. V17-5: instructor title uses user.name. V17-6: ILIKE wildcards escaped. V17-7: data-session attribute removed. V17-8: studio address centralized in @stillwater/config/site. V17-10: SSE per-IP rate limiting. V19: 22 fixes (sand tokens, tw-animate-css, public dir, eager-loads, slug-as-name, email URLs, PaymentFailed payload, honeypot, outline fix, dead code removal). V20: 7 fixes (globals.css @layer base, /schedule nested eager-load, root not-found, auth env fail-fast, dead PostHog env var, pnpm_log.txt untrack, .env.example). (`pnpm build` verified 2026-07-24: 9/9 packages, 20 static pages, 0 errors.)
...
Known test failure: 3 V20-4 env-validator tests in `packages/auth` fail (timeout from drizzle adapter initialization during `vi.resetModules()` + dynamic import). The fail-fast logic in `config.ts` is correct — only the test harness needs adjustment.

---

## Things worth flagging

- **Docs aligned through v20; refreshed 2026-07-24.** Implementation matches the latest codebase. All 13 phases complete + v8→v16-3 audit remediation complete + v17 comprehensive code review remediation complete (11 findings fixed via TDD) + v19 comprehensive 6-axis audit remediation (22 fixes) + v20 CSS cascade + env-validator remediation (7 fixes). See `AUDIT_REMEDIATION.md` for the full v1→v20 remediation history. Live site confirmed operational via agent-browser E2E on 2026-07-19 (V17 fixes committed; V19+V20 committed but deployment status unknown).
...
- **v19 comprehensive 6-axis audit remediation (2026-07-24).** 22 fixes addressing invisible UI elements, slug-as-name bugs, email hardcoded URLs, and more. Key fixes:
  - **V19-1 (Critical):** Defined missing `--color-sand-50` and `--color-sand-100` CSS tokens — 100+ references across codebase were rendering invisible (CTAs, Footer wordmark, stat values).
  - **V19-2:** Installed `tw-animate-css` — Radix overlay animations were silently failing.
  - **V19-3:** Created `apps/web/public/` with favicon + PWA icons (were all 404).
  - **V19-4–V19-6:** Eager-loaded `user` in instructors, members, and schedule routers (slug-as-name and missing instructor name bugs).
  - **V19-13:** Fixed EmailFooter fabricated address (CAN-SPAM compliance).
  - **V19-14:** Added `SITE.url` + replaced 7 hardcoded `https://stillwater.studio` URLs in email templates.
  - **V19-17:** Fixed PaymentFailed email payload mismatch (`customerId` vs `memberId`) — webhook/worker were passing the wrong field, so PaymentFailed emails were NEVER sent.
  - **V19-18–V19-22:** Replaced `focus:outline-none` violations, created `/privacy`/`/terms`/`/accessibility` pages, added honeypot field, removed dead `jobs-client.ts`, increased Checkbox to 24px (WCAG 2.5.8 AA).
- **v20 CSS cascade + env-validator remediation (2026-07-24).** 7 fixes. Most impactful: V20-1 wrapped `packages/ui/src/globals.css` resets in `@layer base` — two unlayered rules (`* { padding: 0 }` + `a { color }`) were overriding EVERY Tailwind v4 utility class site-wide. Also added auth env var fail-fast validator (V20-4) and root `not-found.tsx` (V20-3). 3 V20-4 env-validator tests currently fail (timeout from drizzle adapter initialization during dynamic import).

---

Summary
status_8.md Validation
Mostly accurate — the V19 and V20 commits, fixes, and quality gates are real and verified in the codebase. One factual error:
- "832 tests pass" is wrong — 3 tests fail in packages/auth/src/env-validator.test.ts (V20-4 tests timeout because vi.resetModules() + import('./config') triggers drizzle DB initialization before the env check). Actual: 829 passing, 3 failing, 832 total. The pnpm test command exits with code 1.
Project_Brief.md Updates (16 edits)
Section	Was	Now
Header date/version	post-v17, 2026-07-21	post-v20, 2026-07-24, V19+V20 added to status line
DB schema files	16	17
DB migrations	"5 migrations"	"6 migrations" (correct)
DB tests	17 files, 117 tests	18 files, 131 tests
Auth files	4 test files, 102 tests	5 test files, 107 tests (with failure note)
Auth config.ts	no env-validator	V20-4 env var fail-fast validator noted
Auth resend-client.ts	not listed	Added with V20-4 fail-fast note
API tests	13 files, 118 tests	14 files, 150 tests
Web tests	28 files, 159 tests	38 files, 271 tests
Workers tasks	"11 task files"	"12 task files + barrel index.ts"
Workers tests	11 files, 33 tests	12 files, 45 tests
Quality gates date	2026-07-21	2026-07-24
Test count	~798 passing	832 total (829 passing, 3 failing)
Build output	17 static pages	20 static pages (+3 legal)
Build route description	auth (2)	auth (4: sign-in, sign-out, callback, error)
Things worth flagging	through v17 only	through v20 with V19/V20 fix summaries
"What is NOT yet built"	no known failures	3 V20-4 test failures documented
