...
| 1.12.0  | 2026-07-09 | Super Z / Phase 8 | Active | Phase 8 complete — Background jobs + email templates fully wired. `@stillwater/email` package built (19 source files, 71 tests): 3 shared components (EmailLayout 600px + CAN-SPAM footer, EmailButton primary/ghost sharp corners, EmailFooter studio address + unsubscribe), 13 React Email v6 templates (BookingConfirmation through WeeklyDigest, all with safe hex colors NOT CSS vars), dual-path `send.ts` (`sendEmail` for Server Components local JSX render, `sendEmailNative` for workers Resend Native Templates per ADR-010), 13 send-helpers (type-safe wrappers calling `sendEmailNative` with correct template ID + subject), `template-ids.ts` (13 TEMPLATE_IDS constants + TemplateId type), barrel `index.ts` exporting all public APIs. `@stillwater/workers` package built (12 source files, 33 tests): 11 Trigger.dev v4 tasks (booking-confirmation 30s/3 retries, class-reminder-24h 30s/3, class-reminder-1h 30s/3, class-cancellation-notify 60s/3, waitlist-promotion 30s/3, waitlist-expiry 30s/3, membership-credit-grant 30s/5 retries, membership-expiry-warn 30s/3, payment-failed-notify 30s/3, weekly-digest 120s/2 retries, attendance-summary 60s/2 retries) + barrel `index.ts`. All workers use `sendEmailNative()` via send-helpers (zero React Email 1.8MB bundle bloat per ADR-010). Integration wiring: shared `getJobsClient()` in `@stillwater/config` with stub fallback (TRIGGER_SECRET_KEY not set → stub), `bookings.book` now triggers booking-confirmation + class-reminder-24h + class-reminder-1h (fire-and-forget), `bookings.cancel` job ID fixed from `waitlist.promote` → `waitlist-promotion` (hyphenated per PAD §17.1), `memberships.cancel` sends MembershipCancellation email, `memberships.pause` sends MembershipPaused email, Stripe webhook `invoice.payment_failed` triggers `payment-failed-notify` job (post-commit pattern — only fires after transaction commits). `@trigger.dev/sdk` added as dependency of `@stillwater/config`. `@stillwater/email` added as dependency of `@stillwater/api`. Workers tsconfig: `verbatimModuleSyntax: false` (NodeNext + @stillwater/db CommonJS conflict). 603 tests (109 db + 102 auth + 113 api + 43 payments + 132 web + 71 email + 33 workers); `pnpm check-types` / `pnpm test` / `pnpm build` all green |
| 1.13.0  | 2026-07-10 | Super Z / Phase 9 | Active | Phase 9 complete — Admin surface (RBAC-gated) fully built. 11 admin pages (`/admin` dashboard, `/admin/classes` + `[id]` + `new`, `/admin/schedule`, `/admin/instructors`, `/admin/members` + `[id]`, `/admin/revenue`, `/admin/settings`, `/admin/audit-log`). 9 admin components (AdminShell sidebar+topbar, KpiCard, ClassForm, SessionForm, ScheduleCalendar with @dnd-kit/core, RosterTable with check-in, RevenueChart via Recharts, MemberRoleEditor owner-only). 12 admin tRPC procedures (`listClasses`, `deleteClass`, `listMembers`, `getMemberDetail`, `getRevenueDetails` with real MRR/churn/attendance, `assignRole`, `removeRole`, `listAuditLog`, `getDashboard`, `getRevenue`, `getClassRoster`, `getAttendanceStats`). `audit_log` table added (migration `0003_audit_log_phase9.sql`) with 3 indexes. 7 new shadcn components (table, form, input, textarea, checkbox, calendar, command) — anti-generic patched (rounded-none, no shadows). `cmdk` dependency added. 5 E2E spec files with `skipIf` guards. `lib/admin/audit-log.ts` helper. All admin mutations audit-logged. 2-layer auth defense-in-depth via nested layouts (revenue=manager+, settings=owner). Editorial Calm design throughout (JetBrains Mono for data labels, Warm Mineral palette, 1px rule lines) |
| 1.14.0  | 2026-07-10 | Super Z / Phase 10 | Active | Phase 10 complete — Observability + performance hardening. Sentry (3 configs: client 10% traces/5% replay/100% error replay, server, edge) + PostHog (18 events, reverse proxy `/_analytics`) + Axiom (structured JSON logs via `lib/observability/logger.ts`) + Checkly (3 synthetic checks: booking-flow, sse-endpoint, api-health). `instrumentation.ts` loads Sentry per runtime. `next.config.ts` patched with `withBundleAnalyzer` + `withSentryConfig`. `lighthouserc.js` (Perf 95+ warn, A11y/SEO/BP 100 error). `error-boundary.tsx` with Sentry capture. |
| 1.15.0  | 2026-07-10 | Super Z / Phase 11 | Active | Phase 11 complete — WCAG AAA audit + SEO + OG images. `robots.ts`, `sitemap.ts`, `manifest.ts`, 4 `opengraph-image.tsx` (default, blog/[slug], classes/[slug], instructors/[slug]) via `@vercel/og`. `seo/JsonLd.tsx` + `lib/seo/schemas.ts` (YogaStudio, Article, Person, Breadcrumb). `lib/a11y/focus-utils.ts` (trapFocus, restoreFocus). `a11y/SkipLink.tsx` + `a11y/SrOnly.tsx`. WCAG 2.2 AAA: 7:1 contrast, 44×44px targets, reduced-motion globally, full keyboard nav. |
...
| 1.20.0  | 2026-07-24 | Super Z / V19 Remediation | Active | **V19 remediation — 22 fixes from comprehensive 6-axis audit.** Critical: V19-1 define missing `--color-sand-50`/`--color-sand-100` tokens (invisible CTAs); V19-2 install `tw-animate-css` (broken Radix animations); V19-3 create `apps/web/public/` (favicon + PWA icons); V19-4/5/6 eager-load `user` in instructors/members/schedule routers; V19-7/8/9/10/11 fix 5 page-level bugs (home instructor names, dashboard email, admin schedule, admin instructors, admin member detail $NaN); V19-12 fix 4 slug-replace locations; V19-13 fix EmailFooter fabricated address (CAN-SPAM); V19-14 add `SITE.url` + replace 7 hardcoded URLs in email templates; V19-15 remove misleading HeroNextClass spots indicator; V19-16 apply SITE to admin settings; V19-17 fix PaymentFailed payload mismatch. Medium: V19-18 replace `focus:outline-none` violations; V19-19 create `/privacy`/`/terms`/`/accessibility` pages; V19-20 honeypot field; V19-21 remove dead `jobs-client.ts`; V19-22 checkbox 24px target size. **Doc corrections:** ADR-009 proxy.ts runtime is Node.js default (Edge unsupported) — NOT "inconsistent"; WCAG target rescoped from AAA to AA per W3C guidance; Trigger.dev v3 retirement date corrected. 823 tests (10 config + 47 payments + 150 api + 102 auth + 45 workers + 131 db + 71 email + 267 web). `pnpm check-types` ✅ (9/9), `pnpm lint` ✅ (0 errors), `pnpm test` ✅ (823/823), `pnpm build` ✅ (9/9). |
| 1.21.0  | 2026-07-24 | Super Z / V20 Remediation | Active | **V20 remediation — 7 fixes from V19 live-site E2E + codebase re-validation.** Critical: V20-1 wrap `packages/ui/src/globals.css` resets + link styles in `@layer base` (site-wide CSS cascade bug — unlayered `* { padding: 0 }` + `a { color: var(--color-action) }` were overriding every Tailwind v4 `@layer utilities` rule, breaking all padding/margin/text-color utilities on anchor CTAs, sections, navbar, SkipLink); V20-2 fix `/schedule` page slug-as-name (V19-12 incomplete — only fixed tRPC router, not direct DB query); V20-3 add root `app/not-found.tsx` (custom 404 was never used); V20-4 add auth env var fail-fast validator for `RESEND_API_KEY`/`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`BETTER_AUTH_URL` (was silently falling back to placeholders → production auth 500 outage). Medium: V20-5 remove dead `NEXT_PUBLIC_POSTHOG_HOST` env var; V20-6 untrack `pnpm_log.txt` from git; V20-7 update `.env.example` `BETTER_AUTH_URL` comment. 832 tests (10 config + 47 payments + 150 api + 107 auth + 45 workers + 131 db + 71 email + 271 web). `pnpm check-types` ✅ (9/9), `pnpm lint` ✅ (0 errors), `pnpm test` ✅ (832/832), `pnpm build` ✅ (9/9). |
...
│       │   │   ├── history/                # /history — enrollment history (Phase 6)
...
│       │   ├── export/                  # Phase 6 — CSV utility
│       │   │   ├── csv.ts               # RFC 4180 compliant (escapeCSVField, arrayToCSV, exportToCSV)
│       │   │   └── csv.test.ts          # 6 tests
│       │   └── utils.ts
│       │
...
> **Implementation Status:** ✅ Phase 1 COMPLETE (2026-07-07) + ✅ Phase 9 COMPLETE (2026-07-10). All 18 tables (15 domain entities below including `audit_log` + 3 Better Auth tables in §9.2: `session`, `account`, `verification`) are implemented as Drizzle `pgTable` definitions in `packages/db/src/schema/`. The `members` table includes an additional `stripeCustomerId` column (D6 fix — not shown in ERD below) for Stripe webhook customer lookups. **Phase 9 added `audit_log` table** (migration `0003_audit_log_phase9.sql`) with 3 indexes (staff+created, action, entity) for compliance traceability of all admin mutations. See `packages/db/src/schema/` for the canonical TypeScript definitions and `packages/db/drizzle/migrations/` for the 5 migration SQL files (`0000_dear_dagger.sql` + `0001_equal_iron_lad.sql` + `0002_lyrical_cargill.sql` + `0003_audit_log_phase9.sql` + `0004_huge_hawkeye.sql`).
...
> **Implementation Status:** ✅ Phase 3 COMPLETE (2026-07-07) + ✅ Phase 7 COMPLETE (2026-07-09) + ✅ Phase 9 COMPLETE (2026-07-10). 10 tRPC routers (~42 procedures) implemented in `packages/api/src/routers/`. 4 procedure access tiers (public/protected/staff/owner) in `packages/api/src/trpc.ts`. Booking router uses advisory lock (`pg_advisory_xact_lock`) per ADR-004. Rate limiting on `bookings.book` (10/min via Upstash). Root router merging all 10 routers in `packages/api/src/root.ts`. Web tRPC integration: HTTP handler (`/api/trpc/[trpc]/route.ts`), RSC server caller (`lib/trpc/server.ts`), React client (`lib/trpc/client.tsx`), query key factory (`lib/trpc/query-keys.ts`). Phase 4 added `published == true` filter to `instructors.list` + `getBySlug` (SKILL §7.5.1). Phase 5 confirmed `bookings.book` throws CONFLICT on full session (UI catches and shows WaitlistButton). Phase 6 added `memberships.getMySubscription` `with: { plan: true }` join. Phase 7 unstubbed all Stripe procedures: `memberships.subscribe/cancel/pause/resume` + `payments.getPortalUrl/getInvoices` now call `@stillwater/payments` helpers (previously threw `PRECONDITION_FAILED`). `payments.refund` retained as D12 stub (v1 uses Stripe Dashboard). **Phase 9 added 12 admin procedures**: `admin.listClasses` (paginated + search/filter), `admin.deleteClass` (soft-delete), `admin.listMembers` (paginated + search), `admin.getMemberDetail` (profile + attendance + payment history), `admin.getRevenueDetails` (real MRR + churn + attendance metrics — replaces Phase 3 stub), `admin.assignRole` (ownerProcedure, audit-logged), `admin.removeRole` (ownerProcedure, audit-logged), `admin.listAuditLog` (filterable by date/staff/action), `admin.getDashboard`, `admin.getRevenue`, `admin.getClassRoster`, `admin.getAttendanceStats`. All admin mutations log to `audit_log` table via fire-and-forget pattern. Current test count: 651 (118 api + 102 auth + 117 db + 43 payments + 159 web + 71 email + 41 workers).
...
> **Implementation Status:** ✅ Phase 2 COMPLETE (2026-07-07) + ✅ Phase 9 COMPLETE (2026-07-10). Better Auth v1.6.23 fully configured in `packages/auth/src/config.ts` with Google OAuth + Magic Link plugin + customSession plugin (session enrichment with `memberId` + `roles`). 3 Better Auth schema tables (`session`, `account`, `verification`) in `packages/db/src/schema/auth-tables.ts`. RBAC permission matrix (13 × 6) in `packages/auth/src/rbac.ts`. Server-side auth helpers in `apps/web/src/lib/auth.ts`. 2-layer auth pattern: Layer 1 (proxy.ts, cookie-only) + Layer 2 (4 layout guards). `users.emailVerified` changed from `timestamp` to `boolean` per Better Auth requirement. Migration `0000_dear_dagger.sql`. **Phase 9 added `audit_log` table** (migration `0003_audit_log_phase9.sql`) for compliance traceability. Phase 9 admin surface uses defense-in-depth: `(admin)/layout.tsx` requires staff+, nested `revenue/layout.tsx` requires manager+, nested `settings/layout.tsx` requires owner. Phase 9 added `admin.assignRole` + `admin.removeRole` (ownerProcedure) for role management. All admin mutations audit-logged via `lib/admin/audit-log.ts` (fire-and-forget). 220 tests (102 auth + 107 db + 11 web) at Phase 2; 651 tests total (117 db + 102 auth + 118 api + 43 payments + 159 web + 71 email + 41 workers).
...
> **Implementation Status:** ✅ Phase 9 COMPLETE (2026-07-10). 3 route groups: `(marketing)` public ISR, `(studio)` auth-gated SSR, `(admin)` RBAC-gated SSR. Phase 9 added 11 admin pages, 9 admin components (AdminShell with role-based sidebar, KpiCard, ClassForm, SessionForm, ScheduleCalendar with @dnd-kit/core, RosterTable with check-in, RevenueChart via Recharts, MemberRoleEditor owner-only, SignOutButton). 7 new shadcn primitives added (table, form, input, textarea, checkbox, calendar, command). All admin UI follows Editorial Calm design: rounded-none, no shadows, Warm Mineral palette, JetBrains Mono for data labels. 2-layer auth defense-in-depth: nested layouts enforce role tiers (revenue=manager+, settings=owner).
...
/history                      SSR           No cache        Enrollment history (Phase 6)
...
> **Implementation Status:** ✅ Phase 7 COMPLETE (2026-07-09). The `@stillwater/payments` package is fully built (8 source files, 43 tests): `client.ts` (Stripe SDK singleton, Dahlia API `2026-06-24.dahlia`, null fallback per SKILL §15.20), `types.ts` (7-event discriminated union + `StripeWebhookResult`), `subscriptions.ts` (5 lifecycle helpers: `createCheckoutSession`, `createCustomerPortalSession`, `pauseSubscription`, `resumeSubscription`, `cancelAtPeriodEnd`), `webhooks.ts` (⭐ idempotent handler with `pg_advisory_xact_lock` per ADR-004 + 7 event handlers + race condition handling via unique index on `stripe_event_id`), `invoices.ts` (cursor-based pagination + DTO transformation), `credit-packs.ts` (one-off `mode: 'payment'` checkout), `refunds.ts` (D12 reduced scope — thin wrapper, v1 uses Stripe Dashboard), `index.ts` barrel. Stripe webhook route at `/api/webhooks/stripe/route.ts` (body as TEXT for signature verification, 400/500/200 status codes, `runtime: 'nodejs'`, `dynamic: 'force-dynamic'`). tRPC procedures unstubbed: `memberships.subscribe/cancel/pause/resume` + `payments.getPortalUrl/getInvoices`. `payments.refund` retained as D12 stub. ADR-010 accepted (Resend Native Templates for Trigger.dev workers — relevant to §16.3 email rendering, not payment processing). 5 STRIPE acceptance tests passing (STRIPE-001 through STRIPE-005).
...
> ⚠️ **Paradigm Shift (April 16, 2026):** React Email v6.0.0 unified all component packages into a single `react-email` package. All imports should come from the `react-email` root. `@react-email/components` is deprecated (npm registry deprecation notice on all versions); individual component sub-packages (`@react-email/button`, etc.) are also deprecated. `@react-email/render` remains actively published (v2.0.10, a v6 dependency) but the unified root import `import { render } from 'react-email'` is the recommended pattern. See `react_email_suggestion.md` for full analysis (cited in MEP D43).
...
- **ADR-010** (Accepted 2026-07-09) formally adopts Alternative A for Trigger.dev workers.
...
> ⚠️ **V19 correction (2026-07-24):** Per W3C guidance — *"It is not recommended that Level AAA conformance be required as a general policy for entire sites because it is not possible to satisfy all Level AAA success criteria for some content"* — Stillwater's accessibility target has been **rescoped from WCAG 2.2 Level AAA to WCAG 2.2 Level AA** (the realistic maximum for an entire site). Selective AAA criteria (e.g., 7:1 contrast, 44×44px targets) are still applied where feasible. The ADA Title II legal floor (April 26, 2027) is WCAG 2.1 AA — Stillwater's WCAG 2.2 AA target exceeds this baseline.

The Stillwater platform targets **WCAG 2.2 Level AA** compliance (upgraded from the original AAA target per V19 correction above). This is non-negotiable for the yoga studio demographic (35–65 years, high representation of users with visual impairments, motor limitations, and cognitive considerations).
...
> ⚠️ **V19 correction (2026-07-24):** The original ADR-009 rationale stated "Next.js 16 docs are inconsistent on the default [runtime]" — this has been verified as **incorrect**. Official Next.js 16 docs (`nextjs.org/docs/app/api-reference/file-conventions/proxy`) are explicit and consistent: **"Proxy defaults to using the Node.js runtime."** Edge runtime is **not supported** in `proxy.ts` (cannot be changed). The 2-layer auth pattern remains correct, but the rationale is simplified: proxy.ts runs on Node.js (not Edge), and `auth.api.getSession()` is too expensive for every-request execution regardless.
...
**Rationale:**
- Next.js 16 network-boundary clarification — `proxy.ts` runs on **Node.js runtime by default** (Edge is not supported per official Next.js 16 docs — V19 correction). Cookie-existence-only check via `getSessionCookie()` is fast and runtime-agnostic (no DB access). Full session validation belongs in Server Component layouts via `auth.api.getSession({ headers })`.
...
- Enables the 2-layer auth pattern: `proxy.ts` does cookie-only optimistic check via `getSessionCookie()` (no DB access); Server Component layouts do full validation via `auth.api.getSession({ headers })` + RBAC via `requireRole()`.
...
- `proxy.ts` runs on Node.js runtime (V19 correction — not "Edge or Node.js" as originally stated). The 2-layer pattern exists because calling `auth.api.getSession()` on every request is too expensive. Full validation + RBAC in Server Component layouts via `requireAuth()` / `requireRole()`.

**Rejected:** Keep `middleware.ts` (deprecated in Next.js 16; will be removed). Put full auth + RBAC in proxy.ts (violates Auth0 + Better Auth + Next.js 16 guidance; causes DB round-trip on every request; defeats proxy's purpose as lightweight gating layer).

**Source:** `scaffolding_files.md` preamble (lines 1–9); Next.js 16 blog post (`https://nextjs.org/blog/next-16#proxy`); Next.js 16 docs (`https://nextjs.org/docs/app/api-reference/file-conventions/proxy`); `guide_auth-v5_vs_better-auth.md` §Route Protection Pattern Changes; `MASTER_EXECUTION_PLAN.md` §2 discrepancy D2, D36.
