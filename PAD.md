...
> **Document Status:** `ACTIVE — v1.15.0`
...
| 1.13.0  | 2026-07-10 | Super Z / Phase 9 | Active | Phase 9 complete — Admin surface (RBAC-gated) fully built. 10 admin pages (`/admin` dashboard, `/admin/classes` + `[id]` + `new`, `/admin/schedule`, `/admin/instructors`, `/admin/members` + `[id]`, `/admin/revenue`, `/admin/settings`, `/admin/audit-log`). 9 admin components (AdminShell sidebar+topbar, KpiCard, ClassForm, SessionForm, ScheduleCalendar with @dnd-kit/core, RosterTable with check-in, RevenueChart via Recharts, MemberRoleEditor owner-only). 8 new admin tRPC procedures (`listClasses`, `deleteClass`, `listMembers`, `getMemberDetail`, `getRevenueDetails` with real MRR/churn/attendance, `assignRole`, `removeRole`, `listAuditLog`). `audit_log` table added (migration `0003_audit_log_phase9.sql`) with 3 indexes. 7 new shadcn components (table, form, input, textarea, checkbox, calendar, command) — anti-generic patched (rounded-none, no shadows). `cmdk` dependency added. 5 E2E spec files with `skipIf` guards. `lib/admin/audit-log.ts` helper. All admin mutations audit-logged. 2-layer auth defense-in-depth via nested layouts (revenue=manager+, settings=owner). Editorial Calm design throughout (JetBrains Mono for data labels, Warm Mineral palette, 1px rule lines) |
...
> **Implementation Status:** ✅ Phase 3 COMPLETE (2026-07-07) + ✅ Phase 7 COMPLETE (2026-07-09) + ✅ Phase 9 COMPLETE (2026-07-10). 10 tRPC routers (~38 procedures) implemented in `packages/api/src/routers/`. 4 procedure access tiers (public/protected/staff/owner) in `packages/api/src/trpc.ts`. Booking router uses advisory lock (`pg_advisory_xact_lock`) per ADR-004. Rate limiting on `bookings.book` (10/min via Upstash). Root router merging all 10 routers in `packages/api/src/root.ts`. Web tRPC integration: HTTP handler (`/api/trpc/[trpc]/route.ts`), RSC server caller (`lib/trpc/server.ts`), React client (`lib/trpc/client.tsx`), query key factory (`lib/trpc/query-keys.ts`). Phase 4 added `published == true` filter to `instructors.list` + `getBySlug` (SKILL §7.5.1). Phase 5 confirmed `bookings.book` throws CONFLICT on full session (UI catches and shows WaitlistButton). Phase 6 added `memberships.getMySubscription` `with: { plan: true }` join. Phase 7 unstubbed all Stripe procedures: `memberships.subscribe/cancel/pause/resume` + `payments.getPortalUrl/getInvoices` now call `@stillwater/payments` helpers (previously threw `PRECONDITION_FAILED`). `payments.refund` retained as D12 stub (v1 uses Stripe Dashboard). **Phase 9 added 8 admin procedures**: `admin.listClasses` (paginated + search/filter), `admin.deleteClass` (soft-delete), `admin.listMembers` (paginated + search), `admin.getMemberDetail` (profile + attendance + payment history), `admin.getRevenueDetails` (real MRR + churn + attendance metrics — replaces Phase 3 stub), `admin.assignRole` (ownerProcedure, audit-logged), `admin.removeRole` (ownerProcedure, audit-logged), `admin.listAuditLog` (filterable by date/staff/action). All admin mutations log to `audit_log` table via fire-and-forget pattern. Current test count: 603+ (119+ api + 102 auth + 109+ db + 43 payments + 139+ web + 71 email + 33 workers).
...
> **Implementation Status:** ✅ Phase 2 COMPLETE (2026-07-07) + ✅ Phase 9 COMPLETE (2026-07-10). Better Auth v1.6.23 fully configured in `packages/auth/src/config.ts` with Google OAuth + Magic Link plugin + customSession plugin (session enrichment with `memberId` + `roles`). 3 Better Auth schema tables (`session`, `account`, `verification`) in `packages/db/src/schema/auth-tables.ts`. RBAC permission matrix (13 × 6) in `packages/auth/src/rbac.ts`. Server-side auth helpers in `apps/web/src/lib/auth.ts`. 2-layer auth pattern: Layer 1 (proxy.ts, cookie-only) + Layer 2 (4 layout guards). `users.emailVerified` changed from `timestamp` to `boolean` per Better Auth requirement. Migration `0000_dear_dagger.sql`. **Phase 9 added `audit_log` table** (migration `0003_audit_log_phase9.sql`) for compliance traceability. Phase 9 admin surface uses defense-in-depth: `(admin)/layout.tsx` requires staff+, nested `revenue/layout.tsx` requires manager+, nested `settings/layout.tsx` requires owner. Phase 9 added `admin.assignRole` + `admin.removeRole` (ownerProcedure) for role management. All admin mutations audit-logged via `lib/admin/audit-log.ts` (fire-and-forget). 220 tests (102 auth + 107 db + 11 web) at Phase 2; 603+ tests total at Phase 9.
...
> **Implementation Status:** ✅ Phase 9 COMPLETE (2026-07-10). 3 route groups: `(marketing)` public ISR, `(studio)` auth-gated SSR, `(admin)` RBAC-gated SSR. Phase 9 added 10 admin pages, 9 admin components (AdminShell with role-based sidebar, KpiCard, ClassForm, SessionForm, ScheduleCalendar with @dnd-kit/core, RosterTable with check-in, RevenueChart via Recharts, MemberRoleEditor owner-only, SignOutButton). 7 new shadcn primitives added (table, form, input, textarea, checkbox, calendar, command). All admin UI follows Editorial Calm design: rounded-none, no shadows, Warm Mineral palette, JetBrains Mono for data labels. 2-layer auth defense-in-depth: nested layouts enforce role tiers (revenue=manager+, settings=owner).
