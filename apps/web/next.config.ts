...
const nextConfig: NextConfig = {
  ...
  // ── Security Headers ───────────────────────────────────────────
  // CSP fallback: a static CSP with 'unsafe-inline' for script-src is set
  // here as a SAFETY NET. The per-request nonce-based CSP in proxy.ts
  // (commit 8a1765d) will OVERRIDE this with a stricter version when it
  // runs. If proxy.ts doesn't run for some reason, this fallback still
  // provides XSS mitigation while allowing RSC streaming inline scripts.
  //
  // The nonce-based CSP in proxy.ts is the preferred path:
