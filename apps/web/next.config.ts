...
const nextConfig: NextConfig = {
  ...
  // ── Security Headers ───────────────────────────────────────────
  // CSP is set in proxy.ts (with per-request nonce) — NOT here.
  // The static headers() config can't use per-request values, so CSP
  // must be set in middleware where we can read the nonce generated by
  // experimental.nonce: true.
  headers() {
    return [
      {
        source: "/(.*)",
