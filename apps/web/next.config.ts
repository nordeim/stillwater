...
const nextConfig: NextConfig = {
  ...
  // ── Security Headers ───────────────────────────────────────────
  // NOTE: Content-Security-Policy is NOT set here. Per Next.js docs:
  //   "If two headers match the same path and set the same header key,
  //    the last header key will override the first."
  // `next.config.ts headers()` runs AFTER `proxy.ts`, so any CSP set
  // here would OVERRIDE the per-request nonce-based CSP generated by
  // proxy.ts. That was the root cause of audit finding S1 (v8): the
  // live site served `'unsafe-inline'` because this static CSP won.
  //
  // proxy.ts is the SOLE source of the Content-Security-Policy header:
  //   script-src 'self' 'nonce-${nonce}' 'strict-dynamic' https://js.stripe.com
  // The other security headers below don't conflict with proxy.ts, so
  // they remain here.
  // Source: Stillwater Audit Report v1.0 §7.2 + §8.3 (S1 fix);
